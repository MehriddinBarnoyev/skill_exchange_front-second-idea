    ...
    <div className="container mx-auto p-4 bg-gray-50 min-h-screen">
        ...
        <div className="absolute bottom-0 left-0 w-full transform translate-y-1/2 px-6">
          <div className="flex flex-col md:flex-row items-center">
            <Avatar className="h-32 w-32 border-4 border-white shadow-xl">
              ...
              <AvatarFallback className="text-2xl bg-gray-200">{user.name.charAt(0)}</AvatarFallback>
            </Avatar>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-20">
            ...
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {user.profession && <p className="text-gray-600 mt-1">{user.profession}</p>}
                </div>
                <ConnectionRequestButton receiverId={user.id} className="bg-blue-600 hover:bg-blue-700" receiverName={""} />
              </div>
            </CardHeader>
            <CardContent>
              {user.bio && (
              ...
              {user.skills && user.skills.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {user.skills.map((skill: Skill) => (
                    <Card key={skill.id} className="bg-white border shadow-sm hover:shadow-md transition-shadow">
        ...
        <div>
          <Card className="shadow-md">
            ...
            <CardContent>
              {user.interests ? (
                ...
                <p className="text-gray-500 text-center py-4">No interests listed yet.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="mt-6 shadow-md">
            <CardHeader>
              <CardTitle className="text-xl font-semibold">Member Since</CardTitle>
            </CardHeader>
            <CardContent>
